...
| Contain incident | Pre-state recorded, action taken, and post-state **independently verified**. |
...
> need to re-analyse, and in a real incident belongs after forensic acquisition rather than
...
Purpose: return to normal service without re-opening the attack path.
...
> possibly agent behaviour. Re-run the simulation and confirm the rules still fire. This is the
