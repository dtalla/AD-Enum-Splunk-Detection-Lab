...
Huntress's central point about this intrusion is that `Untitled1.ps1` was a **single use, AI
generated artifact**. Its filenames, its cmdlet choices and its hash will never be seen again.
A detection built on them detects nothing but history.
...
Exfiltration rule nearly failed this test: Sysmon network events often carry no user, which is
why it carries an asset to owner lookup. See `Detections/Risk-rules/README.md`.
...
its constraints, or `summariesonly=true` is hiding unsummarised data. Drop the flag to confirm
the raw mapping, then reenable it for anything scheduled.
