...
| stats sum(risk_score)         as total_risk,    /* HOW MUCH evidence, severity-weighted  */
        ...
        values(risk_message)    as reasons,       /* every contributing rule's one-liner   */
        values(src)             as src,
        values(dest)            as dest,
...
in, prush's session enumerates, stages, archives and exfiltrates. In this single-host lab
the destination is constant too, so either would work here. Identity is the choice that
still works after lateral movement in a real environment, which is the only reason to
prefer it.
...
| 2 | Risk rule cron `*/5` — the rule has to run | 0–5 min |
| 3 | `tstats` rules only look at `-10m@m` → `-5m@m` | +5–10 min for those two stages |
| 4 | Risk event written by `collect`, then indexed | seconds |
| 5 | Correlation cron `*/15` — it has to run | 0–15 min |
...
**20–30 minutes**, and none of that is a fault. It is the sum of deliberate choices.
...
next 15-minute collection. Nothing is broken — you are just watching a clock that was never
synchronised to the person signing.
...
**A 30-minute window on a 5-minute cron.** The overlap is deliberate: a forwarder hiccup,
clock skew or a slow detonation would drop an event out of a tight non-overlapping window
entirely. The overlap is insurance against missing data, and the price is the over-counting
that `new_events_only` then removes. **Keep the overlap, dedupe on write** — that is the
production answer, not "shrink the window until it stops double counting".
...
60-minute window. The throttle key is `risk_object`, not the whole result — so prush being
throttled does not suppress a genuinely new notable for ppitt.

## The three timing bugs we actually hit

**1. Over-counting (fixed by `new_events_only`).** Six overlapping windows meant six risk
...
late-arriving data has landed before the window closes over it. The cost is the extra 5–10
minutes of detection latency shown in the worked example above. That trade — a few minutes
