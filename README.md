...
with pre-compromised credentials, staged tooling in `C:\ProgramData`, and ran an AI-generated
("vibe-coded") PowerShell script — `Untitled1.ps1` — to enumerate Active Directory. The script
only enumerates: it writes one CSV per object class plus an HTML report into
`C:\AD_Reports_<datetime>\` and zips it. Roughly 30 minutes later the attacker dropped
...
| Outbound to Non-Standard Destination | T1567.002 | 30 | `datamodel=Network_Traffic` |

## Response approach

Two SOAR playbooks, split by whether they change anything:

- **`AD_Enum_Enrichment`** — fires automatically on artifact creation, read-only. Pulls the
...
- **`AD_Enum_Isolate_Endpoint_With_Prompt`** — manual, and blocked on a human approval prompt.
...
This repo documents the incident, the detections and the response — **not the lab build**.
Step-by-step VM and endpoint construction stays in a private reference; it answers "can you
build a lab," not "can you handle a threat," and it rots as soon as the config changes.

The one exception is configuration that *is* the detection story — for example, the GPO that
enables PowerShell script block logging, which is what makes 4104 capture possible. Those get
one line each. Test: does this config choice change whether the detection works? Yes → one
line. No → omit.

## Out of scope

**Multi-tool detection validation** — running the same technique through RSAT, `net.exe` and
`ADSISearcher` to prove the rules key on behaviour rather than on tooling — is being done as a
separate project rather than folded in here.
...
| Multi-tool detection validation | separate project |
